	...
	function process_expression (item, index)
	  { var compex = item [0];
		...
		if (syn_list .length > 1)
		  { syn_declarations = cr +indent_string +"var ";
		...
		if (compex .operator .check_flag ("no_equation") || compex .check_flag ("no_equation"))
		  { self .target_text += cr +translated_expression; }
		else
		  { if (syn)
			...
			if (!syn && translated_expression !== target_str_uid) // <-- Exclude tautology ala 'var name = name;'
			  { self .target_text += cr +indent_string +"var " +target_str_uid +" = " +translated_expression +";";
...
var evalresult_raw = current_stack .evalresult,
	evalresult_formatted = cr +"[ " + evalresult_raw .toString () .replace (/,/g,", ") +" ]"; // If result is str with ','?
